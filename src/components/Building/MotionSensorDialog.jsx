...
export default function MotionSensorDialog(props) {
  ...
  React.useEffect(() => {
    ...
    if (props.devices[deviceIdx].type !== "Motion Sensor") {
      return;
    }

    const interval = setInterval(() => {
      getSensor(props.devices[deviceIdx].id)
        .then((res) => {
      ...
      >
        ...
        <DialogContent>
          <Grid container spacing={2}>
            ...
            <Grid item xs={2}>
              {detectedMotion === "on" ? (
                <SensorsIcon sx={{ marginTop: "2vh" }} />
              ) : (
                <SensorsOffIcon sx={{ marginTop: "2vh" }} />
              )}
            </Grid>
          </Grid>
        </DialogContent>
